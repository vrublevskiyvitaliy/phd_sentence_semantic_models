...
from transformers import BertConfig, BertModel, BertForTokenClassification, AdamW, BertConfig, BertTokenizer, AutoTokenizer
...
class BertModelWithPos(BertModel):
    ...
    def forward(
        self,input_ids=None,attention_mask=None,token_type_ids=None,pos_tag_ids=None,
        position_ids=None,head_mask=None,inputs_embeds=None,encoder_hidden_states=None,encoder_attention_mask=None,
...
class BertForSequenceClassificationWithPos(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        config.num_labels = 2
        self.num_labels = 2 # Hardcoded, because of the task
        self.config = config
